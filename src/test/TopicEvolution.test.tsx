...
// Use vi.fn so we can change return values per test
  ...
  const chain: any = new Proxy(
    {},
    {
      get(_target, prop) {
        ...
        // Any other method returns the chain itself
        return () => chain;
      },
    }
  );
...
function setupMock() {
  mockFrom.mockImplementation((table: string) => {
    if (table === "medical_domain_map") return createQueryChain([...domainData]);
    return createQueryChain([...errorData]);
  });
}
...
beforeEach(async () => {
  ...
  domainData = [];
  errorData = [];
  TopicEvolution = (await import("@/components/dashboard/TopicEvolution")).default;
});
