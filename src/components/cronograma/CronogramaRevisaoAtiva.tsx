...
import { ArrowLeft, CheckCircle2, Send, Brain, Stethoscope } from "lucide-react";
import type { Revisao, TemaEstudado } from "@/pages/CronogramaInteligente";
...
const CronogramaRevisaoAtiva = ({ revisao, onComplete, onBack }: Props) => {
  ...
  const taxa = feitas > 0 ? Math.round((acertos / feitas) * 100) : 0;
          ...
          <div className="space-y-4">
            ...
            {feitas > 0 && (
              <div className="rounded-lg bg-secondary/50 p-4">
                <div className="grid grid-cols-3 gap-4 text-center">
                  ...
                  <div>
                    <div className={`text-lg font-bold ${taxa >= 80 ? "text-emerald-500" : taxa >= 60 ? "text-amber-500" : "text-destructive"}`}>
                      {taxa}%
                    </div>
                ...
                {taxa < 60 && (
                  <p className="text-xs text-destructive mt-2 text-center">⚠️ Revisões extras D2 e D5 serão adicionadas</p>
                )}
                {taxa >= 60 && taxa < 80 && (
                  <p className="text-xs text-amber-500 mt-2 text-center">📌 Revisão extra D5 será adicionada</p>
                )}
              </div>
            )}
            <div className="flex gap-2">
              ...
              <Button onClick={() => onComplete(revisao, feitas, erradas)} disabled={feitas === 0} className="flex-1">
                <Send className="h-4 w-4 mr-2" /> Concluir Revisão
              </Button>
            </div>
          </div>
        )}
